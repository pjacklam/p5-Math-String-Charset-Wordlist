...
is($a->file(), 't/testlist.lst');

# create directly
$a = $c->new( { file => 't/testlist.lst' } );
is($a->error(), "");
is(ref($a), $c);
is($a->isa('Math::String::Charset'), 1);
is($a->file(), 't/testlist.lst');
...
for my $list (qw< t/testbig.lst t/test0d.lst >)
  {
...
$a = $c->new( { file => 't/empty.lst' } );
is($a->error(), "");
is(ref($a), $c);
is($a->isa('Math::String::Charset'), 1);
is($a->file(), 't/empty.lst');
